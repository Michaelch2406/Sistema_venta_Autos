...
try {
    if ($_SERVER['REQUEST_METHOD'] === 'GET' && isset($_GET['accion'])) {
        ...
        if ($accion === 'obtener_detalle_vehiculo_nuevo') {
            $veh_id = filter_input(INPUT_POST, 'veh_id', FILTER_VALIDATE_INT);
            if (!$veh_id) {
                $response = ['success' => false, 'message' => 'ID de vehículo no válido (POST).'];
            } else {
                $vehiculo_model = new Vehiculo();
                $imagenes_model = new ImagenesVehiculo_M();

                $detalle_vehiculo = $vehiculo_model->getVehiculoDetalle($veh_id);
                $imagenes_vehiculo = $imagenes_model->getImagenesPorVehiculoId($veh_id);

                if ($detalle_vehiculo) {
                    $response = [
                        'success' => true,
                        'vehiculo' => $detalle_vehiculo,
                        'imagenes' => $imagenes_vehiculo ?: []
                    ];
                } else {
                    $response = ['success' => false, 'message' => 'Vehículo no encontrado o error al obtener detalles (POST).'];
                }
            }
        } else {
            $roles_permitidos_modificar = [1, 2, 3];
